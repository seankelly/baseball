...
impl<'a> Search<'a> {
    ...
    pub fn sort<T: CelSearch + Send>(&self, input: &mut [T], sort_order: &SortOrder) {
    ...
    fn filter_option<T: CelSearch>(&self, element: &T, program: &Program) -> bool {
        ...
        if element.add_cel_variables(&mut player_ctx).is_err() {
            return false;
        }
    ...
    fn sort_key<T: CelSearch>(&self, career: &T, program: &Program) -> f64 {
        let mut player_ctx = self.context.new_inner_scope();
        if career.add_cel_variables(&mut player_ctx).is_err() {
